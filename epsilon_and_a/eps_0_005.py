...
plt.grid(True)
ax = plt.gca()
plt.xlim(0, 5); plt.ylim(-0.6, 1.5)
y_ticks = np.linspace(-0.6, 1.5, 8)
plt.xticks(fontsize=22); plt.yticks(y_ticks, [f"{t:.1f}" for t in y_ticks], fontsize=22)
ax.text(0, 1, r'(c1)', transform=ax.transAxes, fontsize=32, va='top', ha='left', fontweight='bold')
ax.text(0.25, 0.96, r'$\boldsymbol{\varepsilon=0.005}$', transform=ax.transAxes,
        fontsize=22, va='top',
        bbox=dict(boxstyle='square', facecolor='white', edgecolor='black', pad=0.2))
plt.tight_layout()
plt.savefig('Time_Series_ε_3.eps', format='eps',dpi = 300, bbox_inches='tight')
...
plt.grid(True)
ax = plt.gca()
ax.set_xlim(-0.6, 1.2); ax.set_ylim(-0.04, 0.2)
x_ticks = np.linspace(-0.6, 1.4, 6)
y_ticks_pp = np.linspace(-0.04, 0.2, 7)
plt.xticks(x_ticks, [f"{t:.2f}" for t in x_ticks], fontsize=22)
plt.yticks(y_ticks_pp, [f"{t:.2f}" for t in y_ticks_pp], fontsize=22)
ax.text(0.77, 1.0, r'(b3)', transform=ax.transAxes, fontsize=34, va='top', ha='left', fontweight='bold')
plt.tight_layout()
plt.savefig('Phase_Portrait_ε_3.eps', format='eps', dpi=300, bbox_inches='tight')
plt.show()
