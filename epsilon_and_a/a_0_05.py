...
plt.plot(t_scaled, v[mask], 'b', linewidth=2.0, label='v')
plt.plot(t_scaled, w[mask], 'r-', linewidth=2.0, label='w')
...
ax.text(0, 1, '(a1)', transform=ax.transAxes, fontsize=34,
        va='top', ha='left', fontweight='bold')
ax.text(0.25, 0.96, r'$\boldsymbol{a=0.05}$', transform=ax.transAxes,
        fontsize=30, va='top',
        bbox=dict(boxstyle='square', facecolor='white',
                  edgecolor='black', pad=0.3))
plt.tight_layout()
plt.show()
...
y_ticks = np.linspace(-0.04, 0.16, 6)
plt.xticks(x_ticks, [f"{tick:.2f}" for tick in x_ticks], fontsize=22)
plt.yticks(y_ticks, [f"{tick:.2f}" for tick in y_ticks], fontsize=22)
ax.text(0.77, 1.0, r'(b1)', transform=ax.transAxes, fontsize=32,
        va='top', ha='left', fontweight='bold')
plt.tight_layout()
plt.savefig('Phase_Portrait_a_1.eps', format='eps',dpi = 300, bbox_inches='tight')
plt.show()
